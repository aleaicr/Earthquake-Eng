function [u,ud,udd,fs,fluencia,ductilidad]=Newmark_No_Lineal_Bilineal(alfa,beta,Tn,xi,dt,u_i,ud_i,uddg,Fy,R,t)
gamma=0.5;
u=zeros(size(t,1),1);
ud=zeros(size(t,1),1);
...
for i=1:size(t,1)-1
    ...
    while abs(R_ton)>R                                                     % Revision de convergencia N-R.
        ...
        if abs(fs(i+1,1)-u(i+1,1)*alfa*k)>Fy*(1-alfa)                      % Corroboramos estado EPP
            fs(i+1,1)=Fy*sign(fs(i+1,1))*(1-alfa)+u(i+1,1)*alfa*k;
